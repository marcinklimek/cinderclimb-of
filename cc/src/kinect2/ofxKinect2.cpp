...
bool IrStream::readFrame()
{
	...
	if (SUCCEEDED(hr))
	{
		...
		hr = p_frame->get_RelativeTime(reinterpret_cast<INT64*>(&frame.timestamp));
		...
		if (SUCCEEDED(hr))
		{
			hr = p_frame->AccessUnderlyingBuffer(reinterpret_cast<UINT*>(&frame.data_size), reinterpret_cast<UINT16**>(&frame.data));
		}
...
void IrStream::setPixels(Frame frame)
{
	...
	const unsigned short *pixels = static_cast<const unsigned short*>(frame.data);
	if(!pixels)
		return;
	int w = frame.width;
	int h = frame.height;
